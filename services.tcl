...
if {[string match "*[string toupper $botnick]*" [string toupper $regnick]]} {
	set c [lindex $arg 0]
        putlog "Requesting UNBAN on $c from $csnick..."
	putserv "PRIVMSG $csnick :UNBAN $c"
# Clear GECOSBANS
        putserv "PRIVMSG $csnick :CLEAR $c GECOSBANS"
        }
}
...
if {[string match "*[string toupper $botnick]*" [string toupper $regnick]]} {
	set c [lindex $arg 0]
        putlog "Requesting INVITE on $c from $csnick..."
...
if {[lindex $arg 1]!="off"} {
	...
	channel set [lindex $arg 0] need-invite "invite [lindex $arg 0]"
        putlog "Now supporting [lindex $arg 0] with Channel Service."
	} {
	channel set [lindex $arg 0] need-op ""
	channel set [lindex $arg 0] need-unban ""	
	channel set [lindex $arg 0] need-invite ""
        putlog "No longer supporting [lindex $arg 0] with Channel Service."
}
}
...
if {$from==$nsserv} {
        ...
        putlog "$nsnick wants me to identify... identifying..." }
	}
...
if {$from==$csserv} {
        if {[lindex $arg 2]=="identification"} {
               putlog "$nsnick can't recognize me... identifying..."
        	putserv "PRIVMSG $nsnick :IDENTIFY $nspass" }
	}
...
putlog "Services Support loaded."
